...
<?php if( $_SESSION['access']=='Yes' && $_SESSION['user']!='admin'){?>
  ...
  <thead>  
    <tr>
      ...
      <th scope="col">Update Approval</th>
  ...
  <?php
    include ('conn.php');
        $u=$_SESSION['user'];
        $q1="SELECT * from `events` where `Approval` not like 'Yes'";
...
while($d1=mysqli_fetch_assoc($e)){
  ...
  <tbody>
    <tr>
      <th scope="row"><?php echo $d1['Id']?></th>
      <td><?php echo $d1['Name']?></td>
      <td><?php echo $d1['Description']?></td>
      <td><?php echo $d1['Creator']?></td>
      <td><?php echo $d1['Date']?></td>
      <td><?php echo $d1['Stime']?></td>
      <td><?php echo $d1['Etime']?></td>
      <form action='ra.php' method='Post'>
        <td>
            <label>Approval: </label>
            <select id="option" name="option">
        <option id ='opt' value="Yes">Yes</option>
  <option id='opt' value="No">No</option>
  </select>
        </td>
        <td>
            <label>Room: </label>
            <select id="room" name="room">
                ...
                <option id ='opt' value="A">Room A</option>
                <option id='opt' value="B">Room B</option>
                <option id='opt' value="C">Room C</option>

  </select>
        </td>
  <td><button  name = 'id' id='id' type='submit' value=<?php echo $d1['Id']?>>Update</button></td>
