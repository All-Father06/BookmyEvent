...
<?php if( $_SESSION['access']=='Yes' && $_SESSION['user']!='admin'){?>
  ...
  <?php
    include ('conn.php');
        $u=$_SESSION['user'];
        $q1="SELECT * from `user` where `Type` not like 'Admin'";
  ...
  <tbody>
    <tr>
      <th scope="row"><?php echo $d1['emp_id']?></th>
      <td><?php echo $d1['emp_name']?></td>
      <td><?php echo $d1['type']?></td>
      <td><?php echo $d1['access']?></td>
  ...
  <td><button name='id'  id='id' type='submit'  value=<?php echo $d1['emp_id']?>>Update</button></td>
